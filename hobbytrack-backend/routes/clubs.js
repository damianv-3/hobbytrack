...
router.get('/', asyncHandler(async (req, res) =>
{
  ...
  const [clubs] = await db.query(query, [...params, parseInt(limit), offset]);
  const [countResult] = await db.query(countQuery, params);

  res.json({ clubs, pagination: buildPagination(page, limit, countResult[0].total) });
}));
...
router.get('/user/:userId', asyncHandler(async (req, res) =>
{
  ...
  const [countResult] = await db.query('SELECT COUNT(*) AS total FROM club_members WHERE user_id = ?', [userId]);

  res.json({ clubs, pagination: buildPagination(page, limit, countResult[0].total) });
}));

router.post('/:id/join', verifyToken, asyncHandler(async (req, res) =>
{
  const { id } = req.params;
  const userId = req.user.userId;
  ...
  await db.query('INSERT INTO club_members (club_id, user_id, role) VALUES (?, ?, ?)', [id, userId, 'member']);
  res.status(201).json({ message: 'Joined club' });
}));

router.delete('/:id/leave', verifyToken, asyncHandler(async (req, res) =>
{
  const { id } = req.params;
  const userId = req.user.userId;

  const [membership] = await db.query('SELECT * FROM club_members WHERE club_id = ? AND user_id = ?', [id, userId]);
...
router.post('/:id/meetings', verifyToken, asyncHandler(async (req, res) =>
{
  const { id } = req.params;
